...
def generate_physical_environments(num_scenes, script, model="gemini"):
    ...
    - Maximum number of physical environments is 3
        ...
        for vid_idx, duration in enumerate(video_durations, 1):
            ...
            frame_data = next(
                (fr for fr in frame_generation_results 
                 if fr["scene_number"] == scene["scene_number"] and fr["video_index"] == vid_idx - 1),
                None
            )
            
            if not frame_data:
                print(f"Warning: No frame data found for scene {scene['scene_number']}, video {vid_idx}")
                continue
...
def generate_narration_audio(narration_text, target_duration):
    ...
    Uses a lock to ensure only one generation happens at a time.
    Returns the path to the processed audio file.
    """
    with narration_lock:  # Ensure only one thread can generate audio at a time
        try:
            # Check if audio has already been generated for this timestamp
...
def main():
    ...
    env_generator = SceneEnvironmentGenerator()
